...
export type AdminTab = "pending" | "verified" | "rejected";
...
// AdminQueueMobile.dc.html) shell for the whole Admin Command Center epic (CHH-72). Only the
// "pending" tab and "Verifications" nav item are functional — "Verified"/"Rejected" have no
// backing endpoint yet (CHH-75) and "Users" has none either (CHH-76), so both render disabled
  ...
  return (
    <div className="flex min-h-screen flex-col bg-sand font-sans text-ink md:flex-row">
      ...
      <aside className="hidden w-[248px] shrink-0 flex-col gap-1 border-r border-line bg-sand-2 p-3.5 md:flex">
        ...
        <div
          aria-current={activeTab !== "verified" && activeTab !== "rejected" ? "page" : undefined}
          className="flex h-11 items-center gap-2.5 rounded-md bg-cream px-3 text-sm font-bold text-clay shadow-sm"
        >
          <ShieldIcon />
          Verifications
        ...
        <div
          className="flex h-11 cursor-not-allowed items-center gap-2.5 rounded-md px-3 text-sm font-semibold text-ink-2 opacity-60"
          title="User management ships in a later story (CHH-76)"
        >
          <UsersIcon />
          Users
        </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex h-[58px] shrink-0 items-center gap-2.5 border-b border-line bg-cream px-4 md:hidden">
          <h1 className="flex-1 text-[17px] font-extrabold tracking-tight">Verifications</h1>
        ...
        <div className="hidden h-[78px] shrink-0 items-center gap-3.5 border-b border-line bg-cream px-7 md:flex">
          <h1 className="m-0 text-[28px] font-extrabold leading-[34px] tracking-tight">Facility verifications</h1>
          {activeTab === "pending" && typeof pendingCount === "number" && (
        ...
        {/* Tab bar — shared markup across breakpoints */}
        ...
        <nav className="flex h-[66px] shrink-0 items-center border-t border-line bg-cream px-1.5 md:hidden">
          <span className="flex flex-1 flex-col items-center gap-1 text-[11px] font-bold text-clay">
