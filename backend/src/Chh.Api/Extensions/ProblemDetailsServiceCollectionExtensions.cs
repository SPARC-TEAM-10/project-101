...
public static class ProblemDetailsServiceCollectionExtensions
{
    ...
    public static IServiceCollection AddChhProblemDetails(this IServiceCollection services, IWebHostEnvironment environment)
    {
        ...
        Hellang.Middleware.ProblemDetails.ProblemDetailsExtensions.AddProblemDetails(services, options =>
        {
            ...
            // CHH-F02: individual registration domain exception mappings.
            options.Map<MobileNumberNotVerifiedException>(ex =>
                new StatusCodeProblemDetails(StatusCodes.Status422UnprocessableEntity)
                {
                    Detail = ex.Message
                });
            options.Map<IndividualAlreadyRegisteredException>(ex =>
            ...
            // CHH-44: manual attendance marking domain exception mappings.
            options.Map<AlreadyAttendedException>(ex =>
                new StatusCodeProblemDetails(StatusCodes.Status409Conflict)
                {
                    Detail = ex.Message
                });
            options.Map<AttendanceOutsideWindowException>(ex =>
                new StatusCodeProblemDetails(StatusCodes.Status422UnprocessableEntity)
                {
                    Detail = ex.Message
                });
            options.Map<RsvpNotEligibleForAttendanceException>(ex =>
                new StatusCodeProblemDetails(StatusCodes.Status422UnprocessableEntity)
                {
                    Detail = ex.Message
                });
