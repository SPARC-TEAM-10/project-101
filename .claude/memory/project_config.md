...
| Feature Branch | feature/CHH-12-individual-registration |
...
- **Git Base Branch** — used by the Git Branch Skill as the branch-from target when creating feature branches; confirmed to exist locally and on `origin` (github.com/SPARC-TEAM-10/project-101). Corrected 2026-09-07 from `main` to `develop` — actual repo practice (all recent `feature/*` branches merge into `develop`; `main` only receives periodic promotion merges from it via PR).
